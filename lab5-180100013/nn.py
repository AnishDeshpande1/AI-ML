...
class NeuralNetwork:
    ...
    def computeLoss(self, Y, predictions):
        ...
        # TODO 
        #print(np.shape(Y), np.shape(predictions[-1]), "loss")
        return -1*(np.sum(np.multiply(Y,np.log(predictions[-1])))/np.shape(Y)[0])
    ...
    def feedforward(self, X):
        ...
        for i in range(len(self.layers)):
            activ.append(self.layers[i].forwardpass(activ[i]))
        return activ
        #raise NotImplementedError
        # END TODO
    ...
    def backpropagate(self, activations, Y):
        ...
        # TODO
        #print(np.shape(Y), "Y", np.shape(activations[-1]), "activ[-1]")
        er = -Y/activations[-1] #start with the last element, 
        if(type(er)==None):
            print("Delta is none in backprop")
        i = len(activations) - 1
        for layer in reversed(self.layers):
            #print(activations[i].shape, "Activations", er.shape, "er")
            er = layer.backwardpass(self.alpha, activations[i-1], er)
            i-=1
        #raise NotImplementedError
        # END TODO